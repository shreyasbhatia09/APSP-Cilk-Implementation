...
// compile with :icpc -O3 -o rdp_papi R-DP.cpp -DCO -AVX -xhost -DUSE_PAPI -I$TACC_PAPI_INC -Wl,-rpath,$TACC_PAPI_LIB -L$TACC_PAPI_LIB -lpapi -DPAPI -DUSE_PAPI
...
int main(int argc, char *argv[]) {
    ...
    for (int iter = 0; iter < 5; iter++){
        ...
        if(retval != PAPI_OK)
        {
            cout<<"cannot add papi event"<<endl;
        }
        PAPI_start(eventset);

...
//        for (int k = 0; k < endu/loopSize; k++) {
//#pragma parallel
//            for (int i = 0; i < endi/loopSize; i++) {
//#pragma ivdep
//                for (int j = 0; j < endj; j++) {
//
//                    pDest[i*NPDQ+j] = MIN((u[i*NPDQ+k]) + (v[k*NP+j]), x[i*NPDQ+j]);
//                }
//                x[i*NPDQ + j] = x_ij;
//            }
//        }
        int loop = 4;
        __m128* pSrc1 = (__m128*) u;
        __m128* pSrc2 = (__m128*) v;
        ...
        for (int k = 0; k < endu; k++) {
            float *vk = v + k * NP; // GCC optimization. So that this is not computed again.
#pragma parallel
            for (int i = 0; i < endi; i++) {
                float *ui = u + i * NPDQ;
                float *xi = x + i * NPDQ;
                __m128 uik0 = _mm_load_ps(ui + k);
                __m128 uik1 = _mm_load_ps(ui + k+4);

#pragma ivdep
                for (int j = 0; j < endj; j+=8) {
                    __m128 xij0 = _mm_load_ps(xi + j);
                    __m128 xij1 = _mm_load_ps(xi + j+4);
                    __m128 vkj0 = _mm_load_ps(vk + j);
                    __m128 vkj1 = _mm_load_ps(vk + j+4);
                    __m128 min0 = _mm_min_ps(_mm_add_ps(uik0, vkj0), xij0); // Choose minimums of original value and k+i
                    __m128 min1 = _mm_min_ps(_mm_add_ps(uik1, vkj1), xij1); // Choose minimums of original value and k+i

                    _mm_store_ps(xi + j, min0); // Store back to memory
                    _mm_store_ps(xi + j+4, min1); // Store back to memory
...
#pragma ivdep
        ...
        cilk_spawn funcD(x + m11, u + m11, v + m11, nn, xi, xj, uj);
        funcD(x + m21, u + m21, v + m11, nn, xi + nn, xj, uj);
        cilk_sync;
        cilk_spawn funcD(x + m12, u + m11, v + m12, nn, xi, xj + nn, uj);
        funcD(x + m22, u + m21, v + m12, nn, xi + nn, xj + nn, uj);
        cilk_sync;
...
//        countMisses(id );
        ...
        cilk_spawn funcD(x + m11, u + m11, v + m11, nn, xi, xj, uj);
        funcD(x + m12, u + m11, v + m12, nn, xi, xj + nn, uj);
        cilk_sync;
...
//        countMisses(id );
        ...
        funcD(x + m11, u + m11, v + m11, nn, xi, xj, uj);
